...
It reads only the body as it stands, so any later edit must keep the `## Pipeline` section no-mistakes wrote; trimming a long description down without carrying that section through fails the check.
