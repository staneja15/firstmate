...
config/afk-refuse  optional standing away-mode refusal; LOCAL, gitignored, not inherited; present and non-blank (or present and unreadable) refuses every away-mode entry and prints its non-blank lines as the reason, bounded at 20 lines with an explicit truncation marker beyond that; see docs/configuration.md "Standing away-mode refusal"
...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .park-since-* .wedge-escalations-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
2. **Bootstrap** - after the ancestry preflight, detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
