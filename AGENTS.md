...
Any later rewrite of a PR body, including shortening it before relaying it, keeps the no-mistakes `## Pipeline` signature section verbatim; the required check re-reads the whole body on every edit and fails the PR the moment that section is dropped.
