...
The status vocabulary and classification predicates (the captain-relevant verb set, declared-pause and park vocabulary, signal/stale tests, and fleet-scan) live in the shared `bin/fm-classify-lib.sh`, the same library the always-on watcher uses for its own triage when afk is off, so both modes read a status line the same way.
Their triage outcomes diverge in one deliberate place: park handling lives in `bin/fm-watch.sh` only, so normal mode absorbs an idle pane behind an already-delivered captain-relevant status while away mode still escalates it.
`docs/architecture.md` "Event-driven supervision" owns what counts as a park and its cadence.
