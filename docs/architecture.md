...
It fires at most once per park spell and is released only by evidence the crew resumed, so it cannot become a repeating wake; a declared `paused:` or `captain-held` wait names its own blocker and keeps the recheck as its only cadence.
