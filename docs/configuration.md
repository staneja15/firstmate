...
`bin/fm-session-start.sh`'s header is the single owner of session-start ordering, composed commands, digest contents, and native Codex completion receipts under `state/.codex-startup/` with their serialization claim.
`docs/sessionstart-nudge.md` owns the native session-open adapter mechanics.
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, and before a park on an already-delivered captain-relevant status escalates once per park spell; stale panes whose crew is neither provably working nor parked surface immediately
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle parked crew is re-confirmed in the watcher or away-mode daemon; a park is a declared external wait, a captain-held transfer, or an already-delivered captain-relevant status
