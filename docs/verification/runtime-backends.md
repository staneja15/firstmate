...
- Claude Code 2.x pads its bare `❯` composer with U+00A0, which is normalized to a space, so an idle composer reads empty while the same padding around real text still reads pending.
